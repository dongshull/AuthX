import 'dart:typed_data';
import 'package:crypto/crypto.dart';
import 'package:authx/utils/base32.dart';

/// TOTP测试服务，用于验证我们的实现是否正确
class TotpTestService {
  /// 使用RFC 6238中的测试用例验证实现
  static bool runRfcTestCases() {
    // RFC 6238中的测试用例
    final testCases = [
      {
        'secret': 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ', // Base32编码的 12345678901234567890
        'counter': 1,
        'expected': '94287082',
        'algorithm': 'SHA1',
        'digits': 8,
      },
      {
        'secret': 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ',
        'counter': 1,
        'expected': '46119246',
        'algorithm': 'SHA256',
        'digits': 8,
      },
      {
        'secret': 'GEZDGNBVGY3TQOJQGEZDGNBVGY3TQOJQ',
        'counter': 1,
        'expected': '90693936',
        'algorithm': 'SHA512',
        'digits': 8,
      },
    ];

    for (final testCase in testCases) {
      final String result = _generateTotp(
        testCase['secret'] as String,
        testCase['counter'] as int,
        testCase['digits'] as int,
        testCase['algorithm'] as String,
      );

      if (result != testCase['expected']) {
        return false;
      }
    }

    return true;
  }

  static String _generateTotp(String secret, int counter, int digits, String algorithm) {
    try {
      // 解码Base32密钥
      final Uint8List key = Uint8List.fromList(Base32.decode(secret));
      final Uint8List counterBytes = _intToBytes(counter);
      
      // 根据算法选择HMAC函数
      final Hmac hmac = _getHmac(algorithm, key);
      final Uint8List hash = Uint8List.fromList(hmac.convert(counterBytes).bytes);
      
      // 动态截断
      final int offset = hash[hash.length - 1] & 0x0F;
      final int binary = ((hash[offset] & 0x7F) << 24) |
          ((hash[offset + 1] & 0xFF) << 16) |
          ((hash[offset + 2] & 0xFF) << 8) |
          (hash[offset + 3] & 0xFF);

      // 生成指定长度的OTP
      final int otp = binary % _pow(10, digits);
      return otp.toString().padLeft(digits, '0');
    } catch (e) {
      return 'ERROR';
    }
  }

  static Hmac _getHmac(String algorithm, Uint8List key) {
    switch (algorithm.toUpperCase()) {
      case 'SHA1':
        return Hmac(sha1, key);
      case 'SHA256':
        return Hmac(sha256, key);
      case 'SHA512':
        return Hmac(sha512, key);
      default:
        return Hmac(sha1, key);
    }
  }

  static Uint8List _intToBytes(int value) {
    final Uint8List bytes = Uint8List(8);
    for (int i = 7; i >= 0; i--) {
      bytes[i] = (value & 0xFF);
      value >>= 8;
    }
    return bytes;
  }
  
  static int _pow(int base, int exp) {
    int result = 1;
    for (int i = 0; i < exp; i++) {
      result *= base;
    }
    return result;
  }
}